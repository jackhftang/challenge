...
In Lalamove, our backend system is composed of a bunch of microservices. These microservices provide RESTful API for each part of the system. We need a series of API automated tests to protect our system.
...
We have APIs to process orders and require you to **protect** the API logic with black box testing. The API does these tasks:
1. Place Order
...
JSON Payload (when you want to place advance order, put "orderAt" as a future time):
{
...
```
1. Returns HTTP 200 with JSON body normally
2. Returns HTTP 404 if the order doesn’t exist in the DB
